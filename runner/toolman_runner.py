...
import copy
import json
from models.toolman import FunctionCallToolman
from runner.base_runner import ModelRunner


class ToolmanRunner(ModelRunner):
    def __init__(self, args, logger):
        super().__init__(args, logger)
        self.model_name = args.model_name
        self.bellman_model = args.bellman_model
        self.model = FunctionCallToolman(self.bellman_model, args.enable_ptc)
    ...
    def get_standard_functions(self, functions):
        ...
        toolman_functions = [{"type": "function", "function": copy.deepcopy(func)} for func in functions]
        for func in toolman_functions:
            func['function']['name'] = self.name_dict[func['function']['name']]
        return toolman_functions

    def get_standard_fc(self, toolman_tool_call):
        tool_call = copy.deepcopy(toolman_tool_call)
    ...
    def run(self, data):
        ...
        toolman_functions = self.get_standard_functions(functions)
                    ...
                    self.model.messages.append(
                        {
                            ...
                            "tool_name": self.name_dict[function_call['name']],
                            "tool_call_id": tool_calls[t].id,
                        }
                    )
                    # add new tool responses to toolman model (resets each turn)
                    self.model.new_tool_responses.append(
                        {
                            "role": "tool_response",
                            "content": json.dumps(temp_obs, ensure_ascii=False),
                            "tool_name": self.name_dict[function_call['name']],
